...
export default function AdminPage() {
  ...
  const [fulfillmentStatus, setFulfillmentStatus] =
    useState<FulfillmentStatus>("paid");
  ...
  const {
    results: fulfillment,
    status: fulfillmentLoadStatus,
    loadMore,
  } = usePaginatedQuery(
    api.admin.fulfillmentQueue,
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      {toast ? (
        <div
          role="status"
          className="mb-4 border border-line bg-sand/60 px-4 py-3 text-sm text-ink"
        >
          {toast}
          <button
            type="button"
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {(["paid", "packed", "shipped", "delivered"] as const).map((status) => (
            <button
              key={status}
              type="button"
              onClick={() => setFulfillmentStatus(status)}
        ...
        <div className="mt-5 grid gap-3">
            ...
            <article key={order._id} className="flex flex-wrap items-center justify-between gap-4 border border-line bg-paper p-4 text-sm">
              ...
              {order.status === "paid" ? (
                ...
                <button
                  ...
                  onClick={() =>
                    void runAction(
                      () => markDelivered({ orderId: order._id }),
          ...
          {fulfillment.length === 0 && fulfillmentLoadStatus !== "LoadingFirstPage" ? (
            <p className="text-sm text-muted">No fulfillment orders in this status.</p>
          ) : null}
          {fulfillmentLoadStatus === "CanLoadMore" ? (
            <button
              type="button"
              className="border border-line px-4 py-2 text-sm text-ink hover:border-accent"
              onClick={() => loadMore(25)}
